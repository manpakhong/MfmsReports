...
import ebsl.mfms.report.models.sos.ExportPatrolPhotoSo;
...
public class TblPatrolInspectionresultDao extends DaoBase<TblPatrolInspectionresultEo>{
	...
	public TblPatrolInspectionresultDao() throws Exception {
		super();
	}
	public TblPatrolInspectionresultDao(String connectionType) throws Exception {
	...
	public List<ExportPatrolInspectionRoutineVo> readByExportPatrolInspectionRoutineSo(Object so) throws Exception{
		...
		try{
			...
			while(rs.next()) {
				...
				// Inspection Report - Date, Route, Location, Remark, PhotoPath
				Date timeAttended = rs.getDate("pr_timeAttended");
				vo.setPatrolDate(timeAttended);
				...
				// Location , Patrol Result Key to Get the Photos List (Remark, PhotoPath)
				int locationKey = rs.getInt("l_key");
				int patrolResultKey = rs.getInt("pr_Key");
				
				ExportPatrolPhotoSo patrolPhotoSo = new ExportPatrolPhotoSo();
				patrolPhotoSo.setLocationKey(locationKey);
				patrolPhotoSo.setPatrolResultKey(patrolResultKey);
				
				TblPatrolphotoDao tblPatrolPhoto = new TblPatrolphotoDao();
				List<TblPatrolphotoEo> tblPatrolPhotoList = tblPatrolPhoto.read(patrolPhotoSo); 
				
				vo.setPatrolPhotoLists(tblPatrolPhotoList);
	...
	public List<TblPatrolInspectionresultEo> read(Object so) throws Exception{
		List<TblPatrolInspectionresultEo> tblPatrolresultEoList = null;
		StringBuilder whereSql = null;
		PreparedStatement preparedStatement = null;
		try{
			if (so instanceof TblPatrolresultSo == false) {
				throw new Exception("so is not an instanceof TblPatrolInspectionresultSo");
			}
			...
			while(rs.next()) {
				if (tblPatrolresultEoList == null){
					tblPatrolresultEoList = new ArrayList<TblPatrolInspectionresultEo>();
				}
				TblPatrolInspectionresultEo eo = new TblPatrolInspectionresultEo();
