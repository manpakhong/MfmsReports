...
public class TblPatrolresultDao extends DaoBase<TblPatrolresultEo>{
	...
	private final String SELECT_JOIN_LOCATION_ROUTEDEF_SQL=
			"select " + 
			"pr.pr_Key " + 
			",pr.pr_SiteKey " + 
			",pr.pr_RouteDefKey " + 
			...
			",pr.pr_SeqNum " + 
			",pr.pr_TimeAttended " + 
			",pr.pr_PersonAttended " + 
			",pr.pr_Reason " + 
			",pr.pr_Completed " + 
			",pr.pr_CreateBy " + 
			",pr.pr_CreateDateTime " + 
			",pr.pr_LastModifyBy " + 
			",pr.pr_LastModifyDateTime " + 
			",pr.pr_Deleted " + 
			",pr.pr_LastModifyTimeForSync " + 
			",pr.pr_PatrolScheduleKey " + 
			",pr.pr_PatrolStatus " + 
			",pr.pr_MinPtDur " + 
			",pr.pr_MinPtDurUnit " + 
			",pr.pr_MaxPtDur " + 
	...
	public List<PatrolResultVo> readPatrolResultVoList(Object so) throws Exception{
		List<PatrolResultVo> patrolresultVoList = null;
		StringBuilder whereSql = null;
		PreparedStatement preparedStatement = null;
		try{
			if (so instanceof ExportPatrolInspectionRoutineSo == false) {
				throw new Exception("so is not an instanceof ExportPatrolInspectionRoutineSo");
			}
			ExportPatrolInspectionRoutineSo exportPatrolInspectionRoutineSo = (ExportPatrolInspectionRoutineSo) so;
			whereSql = new StringBuilder();
			int wcount = 0;
			
			if(exportPatrolInspectionRoutineSo.getSiteKey() != null){
				if (wcount == 0) {
			...
			if(exportPatrolInspectionRoutineSo.getResultStartDate() != null && exportPatrolInspectionRoutineSo.getResultEndDate() != null){
				if (wcount == 0) {
					whereSql.append("where ");
				}
				 else if (wcount > 0) {
			...
			if(exportPatrolInspectionRoutineSo.getRouteKeyList() != null && 
					exportPatrolInspectionRoutineSo.getRouteKeyList().size() > 0){
				if (wcount == 0) {
					whereSql.append("where ");
				}
				 else if (wcount > 0) {
				...
				List<String> routeKeyList = exportPatrolInspectionRoutineSo.getRouteKeyList();
			...
			if(exportPatrolInspectionRoutineSo.getRouteLocationKeyList() != null && 
					exportPatrolInspectionRoutineSo.getRouteLocationKeyList().size() > 0){
				if (wcount == 0) {
					whereSql.append("where ");
				}
				 else if (wcount > 0) {
				...
				List<String> routeLocationKeyList = exportPatrolInspectionRoutineSo.getRouteLocationKeyList();
			...
			int pcount = 1;
			preparedStatement = connection.prepareStatement(SELECT_JOIN_LOCATION_ROUTEDEF_SQL + whereSql.toString());
			
			if(exportPatrolInspectionRoutineSo.getSiteKey() != null){
				preparedStatement.setInt(pcount, exportPatrolInspectionRoutineSo.getSiteKey());
				pcount++;
			}
			if(exportPatrolInspectionRoutineSo.getResultStartDate() != null && 
					exportPatrolInspectionRoutineSo.getResultEndDate() != null){
				preparedStatement.setDate(pcount, miscUtils.convertUtilDate2SqlDate(exportPatrolInspectionRoutineSo.getResultStartDate()));
				pcount++;
				preparedStatement.setDate(pcount, miscUtils.convertUtilDate2SqlDate(exportPatrolInspectionRoutineSo.getResultEndDate()));
				pcount++;
			}
			ResultSet rs = preparedStatement.executeQuery();
			while(rs.next()) {
				if (patrolresultVoList == null){
					patrolresultVoList = new ArrayList<PatrolResultVo>();
				}
				PatrolResultVo vo = new PatrolResultVo();
				Integer prKey = rs.getInt("pr_Key");
				vo.setPrKey(prKey);
				Integer prSiteKey = rs.getInt("pr_SiteKey");
				vo.setPrSiteKey(prSiteKey);
				Integer prRouteDefKey = rs.getInt("pr_RouteDefKey");
				vo.setPrRouteDefKey(prRouteDefKey);
				Integer prGroupNum = rs.getInt("pr_GroupNum");
				vo.setPrGroupNum(prGroupNum);
				Integer prCoLocationKey = rs.getInt("pr_CoLocationKey");
				vo.setPrCoLocationKey(prCoLocationKey);
				Integer prLocationKey = rs.getInt("pr_LocationKey");
				vo.setPrLocationKey(prLocationKey);
				Integer prSeqNum = rs.getInt("pr_SeqNum");
				vo.setPrSeqNum(prSeqNum);
				Date prTimeAttended = rs.getDate("pr_TimeAttended");
				vo.setPrTimeAttended(prTimeAttended);
				Integer prPersonAttended = rs.getInt("pr_PersonAttended");
				vo.setPrPersonAttended(prPersonAttended);
				String prReason = rs.getString("pr_Reason");
				vo.setPrReason(prReason);
				String prCompleted = rs.getString("pr_Completed");
				vo.setPrCompleted(prCompleted);
				Integer prCreateBy = rs.getInt("pr_CreateBy");
				vo.setPrCreateBy(prCreateBy);
				Date prCreateDateTime = rs.getDate("pr_CreateDateTime");
				vo.setPrCreateDateTime(prCreateDateTime);
				Integer prLastModifyBy = rs.getInt("pr_LastModifyBy");
				vo.setPrLastModifyBy(prLastModifyBy);
				Date prLastModifyDateTime = rs.getDate("pr_LastModifyDateTime");
				vo.setPrLastModifyDateTime(prLastModifyDateTime);
				String prDeleted = rs.getString("pr_Deleted");
				vo.setPrDeleted(prDeleted);
				Date prLastModifyTimeForSync = rs.getTimestamp("pr_LastModifyTimeForSync");
				vo.setPrLastModifyTimeForSync(prLastModifyTimeForSync);
				String prPatrolScheduleKey = rs.getString("pr_PatrolScheduleKey");
				vo.setPrPatrolScheduleKey(prPatrolScheduleKey);
				String prPatrolStatus = rs.getString("pr_PatrolStatus");
				vo.setPrPatrolStatus(prPatrolStatus);
				Integer prMinPtDur = rs.getInt("pr_MinPtDur");
				vo.setPrMinPtDur(prMinPtDur);
				Integer prMinPtDurUnit = rs.getInt("pr_MinPtDurUnit");
				vo.setPrMinPtDurUnit(prMinPtDurUnit);
				Integer prMaxPtDur = rs.getInt("pr_MaxPtDur");
				vo.setPrMaxPtDur(prMaxPtDur);
				Integer prMaxPtDurUnit = rs.getInt("pr_MaxPtDurUnit");
				vo.setPrMaxPtDurUnit(prMaxPtDurUnit);
				String routeName = rs.getString("rd_Name");
				vo.setRouteName(routeName);
				String location = rs.getString("l_Name");
				vo.setLocationName(location);
				patrolresultVoList.add(vo);
			}
		}
		catch (Exception e){
			logger.error(getClassName() + ".readPatrolResultVoList() - so=" + so, e);
			throw e;
		} // end try ... catch
		finally {
